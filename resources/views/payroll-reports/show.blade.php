...
<head>
    ...
    <title>Remuneraciones - {{ $employee['full_name'] }} - {{ $monthLabel }}</title>
    <style>
...
<body>
                    ...
                    @if(count($lines) > 0)
                        @foreach($lines as $li)
                            <tr>
                                ...
                                <td class="right">
                                    {{ $li['payment_type'] === 'trato' ? $li['quantity'] : '—' }}
                                </td>
                                <td class="right">
    ...
    {{-- Tablas secundarias: layout 2 columnas --}}
