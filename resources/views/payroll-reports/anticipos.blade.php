...
<head>
    <meta charset="UTF-8">
    <title>Anticipos — {{ $monthLabel }}</title>
    <style>
...
<body>
    ...
    @if(count($rows) === 0)
        <p style="text-align:center; color:#999; padding:20px;">No hay registros de anticipos para este período.</p>
    @else
        <table>
            <thead>
                <tr>
                    ...
                    <th>Método Pago</th>
                    <th>Observaciones</th>
                    <th class="right">Monto Anticipo</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr>
                    ...
                    <td style="color:#666; font-size:8px;">{{ $row['observations'] ?: '—' }}</td>
            ...
            <tfoot>
                <tr>
                    <td colspan="9">TOTAL ANTICIPOS</td>
        ...
        <div class="grand-total" style="margin-top:20px;">
            <div class="grand-total-label">Total Anticipos {{ $monthLabel }}</div>
            <div class="grand-total-value">$ {{ number_format($grandTotal, 0, ',', '.') }}</div>
        </div>
    @endif

    <div class="footer">
        Reporte de Anticipos &bull; {{ $monthLabel }} &bull; Documento generado automáticamente
    </div>
