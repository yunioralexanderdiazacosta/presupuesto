...
// Filtrar facturas disponibles por sucursal seleccionada
...
watch(() => props.form.fertilizer_order_id, (orderId) => {
    filterBranch.value = '';
    if (orderId) {
...
function initializeProducts() {
    ...
    props.form.products = selectedOrder.value.order_products?.map((op, idx) => {
        const productStocks = props.availableStocksByProduct[op.product_id] || [];
        
        // Auto-expandir todos los productos
        expandedProducts.value[idx] = true;
...
const totalHectareas = computed(() => {
    ...
    return selectedOrder.value.order_irrigation_sectors?.reduce((sum, ois) => {
        return sum + parseFloat(ois.surface || 0);
    }, 0) || 0;
});
...
<template>
    <div>
        ...
        <div class="row mb-3">
            <div class="col-md-12">
                <label class="form-label fw-bold">Orden de Fertilizante *</label>
                <select 
                    v-model="form.fertilizer_order_id" 
                    class="form-select"
                    :class="{ 'is-invalid': form.errors.fertilizer_order_id }"
                >
                ...
                <div v-if="form.errors.fertilizer_order_id" class="invalid-feedback">
                    {{ form.errors.fertilizer_order_id }}
                </div>
            </div>
        </div>
        ...
        <div v-if="selectedOrder">
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-4">
                    <label class="form-label text-muted small">Bomba de Riego</label>
                    <div class="fw-bold">{{ selectedOrder.irrigation_pump?.name || 'N/A' }}</div>
                </div>
                <div class="col-md-4">
                    <label class="form-label text-muted small">Sectores de Riego</label>
                    <div class="fw-bold text-primary">{{ selectedOrder.order_irrigation_sectors?.length || 0 }}</div>
                </div>
            </div>
            ...
            <div class="mt-4">
                ...
                <!-- Sectores de Riego -->
                <div v-if="selectedOrder && selectedOrder.order_irrigation_sectors?.length > 0" class="mb-3">
                    <h6 class="mb-2">Sectores de Riego</h6>
                    <div class="d-flex flex-wrap gap-2">
                        <span 
                            v-for="(ois, idx) in selectedOrder.order_irrigation_sectors" 
                            :key="idx"
                            class="badge bg-info"
                        >
                            {{ ois.irrigation_sector?.name }} 
                            <small>({{ ois.surface?.toLocaleString('es-ES', {minimumFractionDigits: 2}) }} ha)</small>
                        </span>
                    </div>
                </div>
        ...
        <div v-else class="alert alert-warning">
            Por favor seleccione una orden de fertilizante para continuar
        </div>
    </div>
</template>
