...
const props = defineProps({ show: Boolean, bonusType: Object });
const emit = defineEmits(['close', 'saved']);

const form = useForm({ name: '', is_active: true });

watch(() => props.bonusType, (item) => {
    if (item) {
        form.name      = item.name      ?? '';
...
function save() {
    form.put(route('monthly-bonus-types.update', props.bonusType.id), {
        onSuccess: () => {
        ...
        <div class="modal-dialog">
            <div class="modal-content" style="background-color: #f8f9fa;">
                <div class="modal-header bg-white border-bottom">
                    <h5 class="modal-title"><i class="fas fa-edit text-warning me-2"></i>Editar Tipo de Bono Mensual</h5>
