...
const getPriceUnitOptions = (product) => {
    const map = { 1: [1, 2, 8], 2: [1, 2, 8], 3: [3, 4], 4: [3, 4], 5: [5], 8: [1, 2, 8] };
    const allowed = map[product.unit_id];
    const options = allowed
        ? (page.props.units || []).filter(u => allowed.includes(u.value))
        : (page.props.units || []);
...
<template>
    <!-- Encabezado: Nivel 3 / Agrupacion CC / CC -->
    <div class="row g-2 mb-3">
        ...
        <div v-if="isInversionOp(props.form.operation_id, $page.props.operations)" class="col-sm-2">
            ...
            <Multiselect
                ...
                class="multiselect-sm"
            />
            <InputError :message="props.form.errors.investment_id" />
        </div>
    </div>
        ...
        <table class="table table-sm table-bordered align-middle mb-0 w-100" style="font-size:0.78rem;">
            ...
            <thead style="background: linear-gradient(135deg, #1a3d6b 0%, #2563a8 100%); color:#fff; font-size:0.72rem; white-space:nowrap;">
                <tr>
                    ...
                    <th class="text-white px-2 py-1">U. Dosis</th>
                    <th class="text-white px-2 py-1">Precio</th>
                    <th class="text-white px-2 py-1">U. Precio</th>
            ...
            <tbody>
                ...
                >
                    ...
                    <!-- Dosis -->
                    <td class="p-1">
                        <input type="number" v-model="product.dose" step="0.01"
                    ...
                    <!-- U. Precio -->
                    <td class="p-1">
                        <select v-model="product.unit_id_price" class="form-select form-select-sm agro-input"
                            @change="product.unit_id = product.unit_id_price">
                            <option value="">--</option>
                    ...
                    <td class="p-1">
                        <div class="d-grid gap-1"
                            ...
                            <button type="button" @click="selectAllMonths(product)"
                                ...
                                style="font-size:0.58rem; height:100%; border-radius:3px; grid-row: 1 / 3;">vv</button>
