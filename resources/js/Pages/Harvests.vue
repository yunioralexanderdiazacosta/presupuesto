...
const edicionTotals = computed(() => {
  const items = filteredHarvests.value;
  return {
    count: items.length,
                        ...
                        <template #body>
                            ...
                            <template v-else>
                                <tr v-for="(harvest, index) in filteredHarvests" :key="index">
                                    <td class="text-muted">{{harvest.id}}</td>
                                    <td>
                                    ...
                                    <td>
                                        <template v-if="harvest.months && harvest.months.length">
                                            {{ (expandAllMonths ? harvest.months : harvest.months.slice(0, MONTH_PREVIEW))
                                                ...
                                                .join(', ') }}<span v-if="!expandAllMonths && harvest.months.length > MONTH_PREVIEW" class="text-muted"> …</span>
                                        </template>
                                        <span v-else class="text-muted">—</span>
                                    </td>
                                    <td>
                                        <template v-if="harvest.cc && harvest.cc.length">
                                            {{ (expandAllCc ? harvest.cc : harvest.cc.slice(0, CC_PREVIEW))
                                                .map(ccId => (props.costCenters.find(c => String(c.value) === String(ccId)) || {}).label || ccId)
                                                .join(', ') }}<span v-if="!expandAllCc && harvest.cc.length > CC_PREVIEW" class="text-muted"> …</span>
                                        </template>
                                        <span v-else class="text-muted">—</span>
                                    </td>
                                    <td>{{ harvest.user ? harvest.user.name : '—' }}</td>
                        ...
                        <div class="mb-3 d-flex align-items-end gap-2 flex-wrap">
                          ...
                          <div class="col d-flex justify-content-end align-items-end gap-1">
                            <ExportExcelButton
                              ...
                              class="btn btn-success btn-md d-flex align-items-center p-0"
                              filename="Cosecha-Detalles.xlsx"
                            />
                          </div>
                        </div>
                        ...
                        <div class="mb-3 d-flex align-items-end gap-2 flex-wrap">
                          ...
                          <div class="col d-flex justify-content-end align-items-end gap-1">
                            <ExportExcelButton
                              ...
                              class="btn btn-success btn-md d-flex align-items-center p-0"
                              filename="Cosecha-GastosPorHectarea.xlsx"
                            />
                          </div>
                        </div>
                            ...
                            <div class="col-md-4 col-lg-2 col-xl-2 col-xxl-2">
                              <div class="card h-100 p-1 small-card">
                                ...
                                <div class="card-body d-flex flex-column justify-content-end py-1 px-2">
                                  <div class="row">
                                    <div class="col">
                                      <p class="font-sans-serif lh-1 mb-1 fs-8 small-card-number">{{props.percentage}}%</p>
                                    </div>
                                  </div>
                                </div>
                              </div>
                        ...
                        <div class="mb-3 row g-2 align-items-end flex-wrap">
                          <div class="col d-flex justify-content-end align-items-end gap-1">
                            <ExportExcelButton
                              ...
                              class="btn btn-success btn-md d-flex align-items-center p-0"
                              filename="Cosecha-ResumenEstado.xlsx"
                            />
                            <ExportPdfButton
                              :data="excelDataResumen"
                              :headers="[
                              ...
                              class="btn btn-danger btn-md d-flex align-items-center p-0"
                              filename="Cosecha-ResumenEstado.pdf"
                            />
                          </div>
                        </div>
