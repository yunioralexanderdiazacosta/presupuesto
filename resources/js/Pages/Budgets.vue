...
const updateBudget = () => {
   form.post(route('budgets.update', form.id), {
        preserveScroll: true,
        onSuccess: () => {
            form.reset();
...
const onDeleted = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.delete(route('budgets.delete', id), {
                preserveScroll: true,
                onSuccess: () => {
...
const onFilter = () => {
    console.log('prueba');
    router.get(route('budgets.index', {term: term.value}), { preserveState: true});  
}
</script>
<template>
    <Head :title="title" />
	<AppLayout>
        <div class="card mb-3">
            <div class="card-header">
                <div class="row flex-between-end">
                    <div class="col-auto align-self-center">
                        <h5 class="mb-0" data-anchor="data-anchor">Presupuestos</h5>
                    </div>
                    <div class="col-auto ms-auto">
                        <div class="d-flex justify-content-end" data-kt-customer-table-toolbar="base">
                            <!--begin::Export-->
                            <a :href="route('budgets.pdf', {term: term})" target="_blank" class="btn btn-light-primary me-3">
            ...
            <div class="card-body pt-0"> 
                ...
                <Table :id="'budgets'" :total="budgets.data.length" :links="budgets.links">
                    <!--begin::Table head-->
                    <template #header>
                        <!--begin::Table row-->
                    ...
                    <!--begin::Table body-->
                    <template #body>
                        <template v-if="budgets.total == 0">
                            <Empty colspan="6" />
                        </template>
                        <template v-else>
                            <tr v-for="(budget, index) in budgets.data" :key="index">
                                <td><span class="fw-bold mb-1">{{budget.name}}</span></td>
                                <td class="text-end">
                                    ...
                                    <!--end::Update-->
                                    <!--begin::Delete-->
                                    <button type="button" v-tooltip="'Eliminar'" @click="onDeleted(budget.id)" class="btn btn-icon btn-active-light-primary w-30px h-30px">
                                        <span class="svg-icon svg-icon-3">
                                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <CreateBudgetModal @store="storeBudget" :form="form" />
        <EditBudgetModal @update="updateBudget" :form="form" />
    </AppLayout>
</template>