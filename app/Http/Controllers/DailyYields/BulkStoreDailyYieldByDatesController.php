...
class BulkStoreDailyYieldByDatesController extends Controller
{
    ...
    public function __invoke(Request $request)
    {
        ...
        DB::transaction(function () use ($request, $user, $teamId, $seasonId, &$saved, &$skipped) {
            foreach ($request->dates as $date) {
                foreach ($request->employees as $entry) {
                    $employeeId = $entry['employee_id'];
                    $rate       = $entry['rate'];

                    // Resolver contrato vigente para esa fecha
                    ...
                    // Saltar si ya existe tarja "al día" para ese empleado en esa fecha
