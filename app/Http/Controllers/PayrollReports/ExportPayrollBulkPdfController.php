...
class ExportPayrollBulkPdfController extends Controller
{
    public function __invoke(Request $request)
    {
        ...
        $employees = Employee::with(['activeContract.companyReason', 'latestContract.companyReason'])
            ->where('team_id', $user->team_id)
            ->whereIn('id', $request->employee_ids)
            ->orderBy('paternal_surname')
            ->get();

        $reports = $employees->map(function ($employee) use ($user, $seasonId, $month, $monthId, $startDate, $endDate, $dates) {
            ...
            $contract = $employee->activeContract ?? $employee->latestContract;
